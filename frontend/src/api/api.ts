...
export const authApi = {
    login: (payload: LoginPayload) =>
        apiFetch<LoginResponse>('/auth/login', { method: 'POST', body: JSON.stringify(payload) }),
    logout: () => apiFetch<{ success: boolean }>('/auth/logout', { method: 'POST' }),
    refresh: () => apiFetch<RefreshResponse>('/auth/refresh', { method: 'POST' }),
    register: (payload: RegisterPayload) =>
        apiFetch<{ success: boolean; user: UserResponse }>('/auth/register', {
            method: 'POST',
...
export const sessionsApi = {
    ...
    create: (payload: CreateSessionPayload) =>
        apiData<Session>('/sessions', { method: 'POST', body: JSON.stringify(payload) }),
    updateStatus: (id: string, status: string) =>
        apiData<Session>(`/sessions/${id}/status`, {
            method: 'PATCH',
            body: JSON.stringify({ status }),
        }),
    addAgenda: (id: string, payload: { title: string; order: number }) =>
        apiData<AgendaItem>(`/sessions/${id}/agenda`, {
            method: 'POST',
            body: JSON.stringify(payload),
        }),
    getStatistics: (id: string) => apiData<unknown>(`/sessions/${id}/statistics`),
    createSummary: (id: string, content: string) =>
        apiData<unknown>(`/sessions/${id}/summary`, {
            method: 'POST',
...
export const votingsApi = {
    ...
    vote: (votingId: string, payload: CastVotePayload) =>
        apiData<void>(`/votings/${votingId}/vote`, {
            method: 'POST',
            body: JSON.stringify(payload),
        }),
