...
export default function ProtectedRoute({
    children,
    roles = ['MEMBER', 'CHAIRPERSON', 'ADMIN'],
}: Props) {
                ...
                <div className="max-w-sm w-full bg-white border border-slate-200 rounded-2xl p-8 text-center shadow-sm">
                    ...
                    <p className="text-sm text-slate-500 mb-1">
                        ...
                        <span className="font-bold text-slate-700">
                            {roles.map((r) => ROLE_LABEL[r]).join(' lub ')}
                        </span>
                    </p>
                    ...
                    <a
                        href="/panel"
                        className="inline-block bg-slate-900 text-white font-bold text-sm px-6 py-2.5 rounded-xl hover:bg-slate-800 transition"
                    >
                        Wróć do panelu
                    </a>
                </div>
