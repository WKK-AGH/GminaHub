            ...
            {item.voting?.map((voting: any) => (
              <div key={voting.id}>
                ...
                {voting.status === 'COMPLETED' && voting.votes && (() => {
                  const forVotes = voting.votes.filter((v: any) => getVoteChoice(v) === 'YES').length;
                  const against  = voting.votes.filter((v: any) => getVoteChoice(v) === 'NO').length;
                  const abstain  = voting.votes.filter((v: any) => getVoteChoice(v) === 'ABSTAIN').length;
                  return (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm mb-6">
            ...
            <div className="flex items-start gap-2.5 text-slate-600">
              ...
              <div>
                ...
                <p className="font-semibold text-slate-900">
                  {new Date(getSessionDate(session)).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
            </div>
            {session.committee && (
