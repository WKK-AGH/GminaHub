...
import { ChevronLeft, ChevronRight, Radio, CalendarDays } from 'lucide-react';
      ...
      <div className="flex items-center gap-4 px-4 py-2.5 border-t border-slate-100 bg-slate-50">
        {Object.entries(STATUS_DOT).map(([status, dot]) => (
          <span key={status} className="flex items-center gap-1.5 text-[10px] text-slate-500">
            <span className={`w-2 h-2 rounded-full ${dot.replace('animate-pulse', '')}`} />
            {STATUS_LABEL[status]}
          </span>
        ))}
        {loading && <span className="text-[10px] text-slate-400 ml-auto">Ładowanie...</span>}
      </div>
          ...
          {selectedSessions.length === 0 ? (
            ...
            <div className="space-y-2">
              {selectedSessions.map(s => (
                <div key={s.id} className="flex items-start justify-between gap-3 bg-slate-50 border border-slate-200 rounded p-3">
                  <div className="flex items-start gap-2.5">
                    <span className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${STATUS_DOT[s.status]?.replace('animate-pulse','') ?? 'bg-slate-400'}`} />
                    <div>
                      <p className="text-sm font-semibold text-slate-900 leading-snug">{s.title}</p>
                      <p className="text-xs text-slate-400 mt-0.5">
                        {new Date(s.scheduledAt).toLocaleTimeString('pl-PL', { hour: '2-digit', minute: '2-digit' })}
                        {' · '}
