    ...
    <nav className="bg-white border-b border-slate-200 sticky top-0 z-50">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-14">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 bg-[#B91C1C] rounded flex items-center justify-center flex-shrink-0">
              <span className="text-white font-bold text-xs leading-none">RG</span>
            </div>
            <div className="leading-tight">
              <span className="block text-[#B91C1C] font-bold text-sm tracking-wide">RADA GMINY</span>
          ...
          <div className="hidden md:flex items-center gap-6">
            {!isPanelRoute && (
              <>
                <a href="/#glosowania"  className="text-sm text-slate-600 hover:text-[#B91C1C] transition font-medium">Rejestr głosowań</a>
                <a href="/#kalendarz"  className="text-sm text-slate-600 hover:text-[#B91C1C] transition font-medium">Kalendarz</a>
                <a href="/#nagrania"   className="text-sm text-slate-600 hover:text-[#B91C1C] transition font-medium">Archiwum nagrań</a>
              </>
            )}
          ...
          <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden p-2 text-slate-600 hover:text-[#B91C1C]">
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 px-4 py-3 space-y-2">
          {!isPanelRoute ? (
            <>
              <a href="/#glosowania"  onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-600 font-medium border-b border-slate-100">Rejestr głosowań</a>
              <a href="/#kalendarz"  onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-600 font-medium border-b border-slate-100">Kalendarz</a>
              <a href="/#nagrania"   onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-600 font-medium border-b border-slate-100">Archiwum nagrań</a>
              <Link to={currentUser ? '/panel' : '/login'} onClick={() => setMenuOpen(false)}
            ...
            <>
              <Link to="/panel" onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-600 font-medium border-b border-slate-100">Panel</Link>
              {currentUser && currentUser.role === 'ADMINISTRATOR' && (
                <Link to="/uzytkownicy" onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-600 font-medium border-b border-slate-100">Użytkownicy</Link>
              )}
              {currentUser && (currentUser.role === 'PRZEWODNICZACY' || currentUser.role === 'ADMINISTRATOR') && (
                <Link to="/komisje" onClick={() => setMenuOpen(false)} className="block py-2 text-sm text-slate-600 font-medium border-b border-slate-100">Komisje</Link>
              )}
