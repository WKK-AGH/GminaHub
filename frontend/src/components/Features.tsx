import { CheckCircle2, XCircle, MinusCircle, TrendingUp, Lock, ArrowRight, BarChart2 } from 'lucide-react';
...
function WynikiBar({ za, przeciw, wstrzymalo, lacznie }: { za: number; przeciw: number; wstrzymalo: number; lacznie: number }) {
  const pct = (n: number) => Math.round((n / lacznie) * 100);
  return (
    <div className="mt-3 space-y-2">
      <div className="flex h-2 rounded-full overflow-hidden bg-slate-100 gap-px">
        <div className="bg-emerald-500 rounded-l-full transition-all" style={{ width: `${pct(za)}%` }} />
        <div className="bg-red-500 transition-all" style={{ width: `${pct(przeciw)}%` }} />
        <div className="bg-slate-300 rounded-r-full transition-all" style={{ width: `${pct(wstrzymalo)}%` }} />
      </div>
    ...
    <div className="bg-slate-50">
      ...
      <section id="glosowania" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
          <div>
            <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-2">Jawność i transparentność</p>
        ...
        <div className="space-y-3">
          {glosowania.map(g => (
            <div key={g.id} className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-slate-300 transition shadow-sm hover:shadow-md">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <StatusBadge status={g.status} />
                    <span className="text-xs text-slate-400">{g.sesja} · {g.data}</span>
                  </div>
                  <p className="font-bold text-slate-900 text-sm leading-snug">{g.tytul}</p>
      ...
      <section className="border-t border-slate-200 bg-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-2">Dane publiczne</p>
            <h2 className="text-2xl font-black text-slate-950 tracking-tight">Aktywność Rady Gminy w 2026 r.</h2>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { icon: <BarChart2 className="w-5 h-5 text-blue-600" />, val: '34',  label: 'Sesji odbytych',      bg: 'bg-blue-50',    border: 'border-blue-100' },
              { icon: <CheckCircle2 className="w-5 h-5 text-emerald-600" />, val: '187', label: 'Uchwał uchwalonych', bg: 'bg-emerald-50', border: 'border-emerald-100' },
              { icon: <XCircle className="w-5 h-5 text-red-500" />,         val: '31',  label: 'Uchwał odrzuconych', bg: 'bg-red-50',     border: 'border-red-100' },
              { icon: <TrendingUp className="w-5 h-5 text-amber-600" />,    val: '94%', label: 'Avg. frekwencja',    bg: 'bg-amber-50',   border: 'border-amber-100' },
            ].map((item, i) => (
      ...
      <section id="nagrania" className="border-t border-slate-200 bg-slate-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
            <div>
              <p className="text-xs font-bold text-blue-600 uppercase tracking-widest mb-2">Publiczne archiwum</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { numer: 'XXXIV', data: '28.05.2026', czas: '3h 12min', status: 'live' as const },
              { numer: 'XXXIII', data: '14.04.2026', czas: '2h 47min', status: 'dostepne' as const },
              { numer: 'XXXII', data: '17.03.2026', czas: '4h 03min', status: 'dostepne' as const },
            ].map((n, i) => (
              <div key={i} className="bg-white border border-slate-200 rounded-2xl p-5 hover:border-slate-300 transition shadow-sm group">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <p className="font-extrabold text-slate-900">{n.numer} Sesja Zwyczajna</p>
                    <p className="text-xs text-slate-400 mt-0.5">{n.data} · {n.czas}</p>
                  </div>
                  {n.status === 'live' ? (
