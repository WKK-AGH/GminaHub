...
export default function Features() {
    ...
    const totalVotings = finishedSessions
        .flatMap((s) => s.agendaItems ?? [])
        .flatMap((a) => a.voting ?? []).length;
    const passedVotings = finishedSessions
        .flatMap((s) => s.agendaItems ?? [])
        .flatMap((a) => a.voting ?? [])
        .filter((v) => {
    ...
    return (
        <div className="bg-slate-50">
                    ...
                    <div className="max-w-6xl mx-auto flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-white font-semibold text-sm">
                                Trwa sesja:{' '}
                                <span className="font-bold">{activeSessions[0].title}</span>
                            </p>
                        </div>
                        <Link
                            to="/login"
                            className="inline-flex items-center gap-2 bg-white text-[#B91C1C] font-bold text-sm px-4 py-1.5 rounded transition hover:bg-red-50"
                        >
                ...
                {loading ? (
                    ...
                    <div className="bg-white border border-slate-200 rounded p-10 text-center text-slate-400">
                        ...
                        <p className="text-xs mt-1">
                            Wyniki głosowań pojawią się po przeprowadzeniu pierwszych sesji.
                        </p>
                    </div>
                ) : (
                    <div className="space-y-2">
                        {finishedSessions.slice(0, 5).map((session) => {
                            const votings = (session.agendaItems ?? []).flatMap(
                                (a) => a.voting ?? [],
                            );
                            const passed = votings.filter((v) => {
                                const yes =
                                    v.votes?.filter((vote: any) => vote.value === 'FOR').length ??
                                    0;
                                const total = v.votes?.length ?? 0;
                                return total > 0 && yes > total / 2;
                            }).length;
                            ...
                            return (
                                <div
                                    key={session.id}
                                    className="bg-white border border-slate-200 rounded p-4 flex items-center justify-between gap-4 flex-wrap hover:border-slate-300 transition"
                                >
                                    <div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                        <div>
                            ...
                            <p className="text-slate-500 text-sm mb-4">
                                Terminy sesji i posiedzeń komisji. Kliknij datę aby zobaczyć
                                szczegóły.
                            </p>
                            ...
                            {!loading && plannedSessions.length > 0 && (
                                <div className="space-y-2 mb-4">
                                    ...
                                    {plannedSessions.slice(0, 3).map((s) => (
                                        <div
                                            key={s.id}
                                            className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded px-4 py-2.5"
                                        >
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-400">
                                                    {formatDate(getSessionDate(s))} ·{' '}
                                                    {formatTime(getSessionDate(s))}
                                                </p>
                                            </div>
                                            <span className="text-xs font-semibold px-2 py-0.5 rounded border text-amber-700 bg-amber-50 border-amber-200">
                                                Zaplanowana
                                            </span>
                                        </div>
                                    ))}
            ...
            <section className="border-t border-slate-200 bg-slate-50 py-14">
                <div className="max-w-6xl mx-auto px-4 sm:px-6">
                    <h2 className="text-xl font-bold text-[#B91C1C] mb-8">
                        Aktywność Rady Gminy Nasza Gmina
                    </h2>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {[
                            {
                                icon: <BarChart2 className="w-4 h-4 text-[#B91C1C]" />,
                                val: loading ? '...' : String(finishedSessions.length),
                                label: 'Sesji odbytych',
                            },
                            {
                                icon: <CheckCircle2 className="w-4 h-4 text-emerald-600" />,
                                val: loading ? '...' : String(passedVotings),
                                label: 'Uchwał przyjętych',
                            },
                            {
                                icon: <XCircle className="w-4 h-4 text-red-400" />,
                                val: loading ? '...' : String(totalVotings - passedVotings),
                            ...
                            <div
                                key={i}
                                className="bg-white border border-slate-200 rounded p-5 text-center"
                            >
                    ...
                    {!loading && (
                        <p className="text-xs text-slate-400 mt-3">
                            Dane aktualizowane na podstawie przeprowadzonych sesji.
                        </p>
                    )}
                </div>
            </section>
            ...
            <section id="nagrania" className="border-t border-slate-200 bg-white py-14">
                <div className="max-w-6xl mx-auto px-4 sm:px-6">
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded p-10 text-center text-slate-400">
                        ...
                        <p className="text-xs mt-1">
                            Nagrania pojawią się tutaj po przeprowadzeniu pierwszych sesji.
                        </p>
                    </div>
                </div>
            </section>
