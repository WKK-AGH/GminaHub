...
import {
  CalendarDays, Bell, LogOut, Clock, FileText,
  ChevronRight, Users, AlertCircle, Radio, Plus,
  BarChart2, Search, X, CheckCircle2
} from 'lucide-react';
...
function SessionCard({ session }: { session: Session }) {
  ...
  return (
    <div className={`bg-white border rounded p-4 hover:border-slate-300 transition ${session.status === 'ACTIVE' ? 'border-l-4 border-l-[#B91C1C] border-slate-200' : 'border-slate-200'}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        <p className="font-semibold text-slate-900 text-sm leading-snug">{session.title}</p>
        <span className={`text-xs font-semibold px-2 py-0.5 rounded border flex-shrink-0 ${cfg.className}`}>{cfg.label}</span>
      </div>
      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 mb-3">
        <span className="flex items-center gap-1"><CalendarDays className="w-3.5 h-3.5" />{formatDate(session.scheduledAt)}</span>
        <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{formatTime(session.scheduledAt)}</span>
        {itemCount > 0 && <span className="flex items-center gap-1"><FileText className="w-3.5 h-3.5" />{itemCount} pkt. agendy</span>}
      </div>
      <div className="flex items-center gap-3 pt-2 border-t border-slate-100 flex-wrap">
        {session.status === 'ACTIVE' && (
          <Link to={`/live/${session.id}`} className="text-xs font-semibold text-[#B91C1C] hover:underline flex items-center gap-1">
        ...
        <Link to={`/sesja/${session.id}`} className="text-xs font-semibold text-slate-500 hover:text-[#B91C1C] flex items-center gap-1 ml-auto">
    ...
    <div className="min-h-[80vh] bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <button onClick={logout} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800 border border-slate-200 px-3 py-1.5 rounded hover:bg-slate-50 transition">
            <LogOut className="w-3.5 h-3.5" /><span className="hidden sm:inline">Wyloguj</span>
          </button>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-7">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-7">
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <h1 className="text-xl font-bold text-[#B91C1C]">Dzień dobry, {currentUser.firstName}</h1>

              </div>
              {canManage && (
                <Link to="/sesja/nowa" className="inline-flex items-center gap-2 bg-[#B91C1C] hover:bg-[#991B1B] text-white font-semibold px-4 py-2 rounded text-sm transition">
            ...
            {loading ? <SkeletonStats /> : (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Sesji łącznie', value: sessions.length,          icon: <CalendarDays className="w-4 h-4" /> },
                  { label: 'Aktywne',       value: activeSessions.length,    icon: <Radio        className="w-4 h-4" /> },
                  { label: 'Nadchodzące',   value: plannedSessions.length,   icon: <Clock        className="w-4 h-4" /> },
                  { label: 'Zakończone',    value: finishedSessions.length,  icon: <BarChart2    className="w-4 h-4" /> },
                ].map((stat, i) => (
                  <div key={i} className="bg-white border border-slate-200 rounded p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold text-[#B91C1C]">{stat.value}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {!loading && activeSessions.length > 0 && (
              <div>
                ...
                <div className="space-y-2">{activeSessions.map(s => <SessionCard key={s.id} session={s} />)}</div>
              </div>
            )}
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-sm font-semibold text-slate-700 uppercase tracking-wide flex items-center gap-2">
                    <CalendarDays className="w-4 h-4 text-[#B91C1C]" /> Nadchodzące sesje
                  </h2>
                  <button onClick={() => setActiveTab('sessions')} className="text-xs text-[#B91C1C] font-semibold hover:underline">
                    Wszystkie →
                  </button>
                </div>
                <div className="space-y-2">{plannedSessions.slice(0, 3).map(s => <SessionCard key={s.id} session={s} />)}</div>
              </div>
            )}
            ...
            {canManage && (
              <div className="bg-white border border-slate-200 rounded p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  <Link to="/sesja/nowa" className="inline-flex items-center gap-1.5 text-sm font-semibold text-white bg-[#B91C1C] hover:bg-[#991B1B] px-4 py-2 rounded transition">
                    <Plus className="w-4 h-4" /> Nowa sesja
                  </Link>
                  <Link to="/komisje" className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#B91C1C] border border-[#B91C1C] hover:bg-red-50 px-4 py-2 rounded transition">
                    <Users className="w-4 h-4" /> Komisje
                  </Link>
                  {hasRole('ADMINISTRATOR') && (
                    <Link to="/uzytkownicy" className="inline-flex items-center gap-1.5 text-sm font-semibold text-slate-600 border border-slate-300 hover:bg-slate-50 px-4 py-2 rounded transition">
        ...
        {activeTab === 'sessions' && (
          <div>
            ...
            {loading && <div className="space-y-2"><SkeletonCard /><SkeletonCard /><SkeletonCard /></div>}
            ...
            {!loading && filteredSessions.length === 0 && (
              <div className="text-center py-12 text-slate-400 border border-dashed border-slate-200 rounded bg-white">
                ...
                <p className="text-sm font-medium">
                  {searchQuery ? `Brak wyników dla „${searchQuery}"` : 'Brak sesji w systemie'}
                </p>
                {searchQuery && (
            ...
            {!loading && filteredSessions.length > 0 && (
              <div>
                <p className="text-xs text-slate-400 mb-3">
                  Znaleziono: {filteredSessions.length} {filteredSessions.length === 1 ? 'sesja' : 'sesji'}
                </p>
                <div className="space-y-2">{filteredSessions.map(s => <SessionCard key={s.id} session={s} />)}</div>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'notifications' && (
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              {unreadCount > 0 && (
                <button onClick={markAllRead} className="text-xs font-semibold text-[#B91C1C] hover:underline">
                  ...
                  <div key={n.id}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-3">
                        <div className={`w-2 h-2 rounded-full mt-1.5 flex-shrink-0 ${n.read ? 'bg-slate-300' : 'bg-[#B91C1C]'}`} />
                        <div>
                      ...
                      {!n.read && (
                        <button onClick={() => markRead(n.id)}
                          className="text-xs text-slate-400 hover:text-[#B91C1C] flex-shrink-0 flex items-center gap-1">
