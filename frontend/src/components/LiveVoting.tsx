...
import { getSessionDate, getVoteChoice } from '../utils/dateUtils';
  ...
  return (
    <div className="space-y-3">
      <div className="flex h-4 rounded-full overflow-hidden gap-0.5">
        <div className="bg-emerald-500 rounded-l-full transition-all duration-700" style={{ width: `${pct(forVotes)}%` }} />
        <div className="bg-red-500   transition-all duration-700" style={{ width: `${pct(against)}%` }} />
        <div className="bg-slate-300 rounded-r-full transition-all duration-700" style={{ width: `${pct(abstain)}%` }} />
      </div>
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: 'ZA',        val: forVotes,         cls: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' },
          { label: 'PRZECIW',   val: against,    cls: 'text-red-500',     bg: 'bg-red-50     border-red-200'     },
          { label: 'WSTRZYMUJĘ',val: abstain, cls: 'text-slate-500',   bg: 'bg-slate-100  border-slate-200'   },
        ].map(r => (
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-4 py-3">
        <div className="max-w-2xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-2 text-xs font-bold px-3 py-1.5 rounded-full ${connected ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
            ...
            <div className="flex items-center gap-1.5 text-xs font-mono text-slate-400 bg-slate-800 px-3 py-1.5 rounded-full">
              <Clock className="w-3 h-3" />{formatCzas(elapsed)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-red-400 uppercase tracking-widest">Aktywna sesja</p>
              <p className="font-extrabold text-white text-base mt-0.5">{session.title}</p>
              <p className="text-xs text-slate-400 mt-1">
                {new Date(getSessionDate(session)).toLocaleDateString('pl-PL', { day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border-2 border-[#B91C1C]/40 rounded-2xl p-6 space-y-5">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold text-red-400 uppercase tracking-widest">Głosowanie otwarte</span>
              </div>
              <h2 className="text-lg font-extrabold text-white">{activeVoting.title}</h2>
              {activeItem && <p className="text-xs text-slate-400 mt-1">Punkt: {activeItem.title}</p>}
            </div>
                ...
                <div className="flex gap-3">
                  {(['FOR', 'AGAINST', 'ABSTAIN'] as VoteValue[]).map(v => (
                    <VoteButton key={v} value={v}
                      ...
                      disabled={sending}
                      onClick={() => setSelectedVote(v === selectedVote ? null : v)}
                    />
                  ))}
                </div>
                <button onClick={handleGlosowanie} disabled={!selectedVote || sending}
                  ...
                  {sending
                    ...
                    : selectedVote ? `Potwierdź: ${VOTE_LABEL[selectedVote]}` : 'Wybierz głos'
                  }
                </button>
              </>
            ) : (
                ...
                <div className="flex items-center gap-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      Twój głos: <span className="font-bold text-white">{selectedVote ? VOTE_LABEL[selectedVote] : ''}</span>
                    </p>
                  </div>
                </div>
                {results && (
              ...
              {agendaItems.sort((a, b) => a.order - b.order).map((item, i) => {
                ...
                return (
                  <div key={item.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition ${isActive ? 'bg-[#B91C1C]/10 border border-[#B91C1C]/30' : 'border border-transparent'}`}>
                    ...
                    <span className={`text-sm font-semibold flex-1 truncate ${isActive ? 'text-red-300' : isCompleted ? 'text-slate-400' : 'text-slate-300'}`}>
                      {item.title}
                    </span>
                    {isActive   && <Radio        className="w-3.5 h-3.5 text-red-400 flex-shrink-0 animate-pulse" />}
