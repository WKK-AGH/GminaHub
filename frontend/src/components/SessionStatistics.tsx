...
import {
  ...
  FileDown, Calendar, Clock, Users, AlertTriangle,
...
function StackedBar({ yes, no, abstain }: { yes: number; no: number; abstain: number }) {
  ...
  return (
    <div className="flex h-3 rounded-full overflow-hidden gap-px bg-slate-100">
      <div className="bg-emerald-500 transition-all duration-700" style={{ width: `${pct(yes, total)}%` }} />
      <div className="bg-red-500   transition-all duration-700" style={{ width: `${pct(no, total)}%` }} />
      <div className="bg-slate-300 transition-all duration-700" style={{ width: `${pct(abstain, total)}%` }} />
    </div>
  );
}
...
function VotingRow({ voting, index }: { voting: any; index: number }) {
  ...
  return (
    <div className={`border rounded-2xl overflow-hidden transition-all ${passed ? 'border-slate-200' : 'border-red-100'}`}>
      <button onClick={() => setExpanded(v => !v)}
        className="w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-slate-50/60 transition">
        <span className="text-xs font-mono font-bold text-slate-400 flex-shrink-0 w-5">{index + 1}.</span>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-slate-900 truncate mb-2">{voting.title}</p>
          <StackedBar yes={yes} no={no} abstain={abstain} />
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          <span className={`text-sm font-extrabold tabular-nums ${passed ? 'text-emerald-600' : 'text-red-500'}`}>
            {yesPct}%
          </span>
          {passed ? <CheckCircle2 className="w-5 h-5 text-emerald-500" /> : <XCircle className="w-5 h-5 text-red-400" />}
        ...
        <div className="border-t border-slate-100 px-5 py-4 bg-slate-50/40">
          <div className="grid grid-cols-3 gap-3 mt-2">
            {[
              ...
              <div key={r.label} className={`flex flex-col items-center px-3 py-2 rounded-xl border text-xs ${r.bg}`}>
                <span className={`text-2xl font-extrabold leading-none ${r.cls}`}>{r.val}</span>
                <span className={`font-bold mt-1 ${r.cls}`}>{r.label}</span>
                <span className="text-slate-400 mt-0.5">{pct(r.val, total)}%</span>
              </div>
            ))}
          </div>
          <p className={`text-xs mt-3 font-semibold ${passed ? 'text-emerald-600' : 'text-red-500'}`}>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <Link to={`/session/${id}`} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-900 font-medium transition">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-8">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold text-blue-600 uppercase tracking-widest">Session statistics</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-950 tracking-tight">{session.title}</h1>
          <div className="flex flex-wrap gap-4 mt-2 text-sm text-slate-500">
            <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-slate-400" />{formatDate(session.scheduledAt)}</span>
            <span className="flex items-center gap-1.5"><Clock    className="w-4 h-4 text-slate-400" />{new Date(session.scheduledAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { icon: <BarChart2   className="w-5 h-5" />, label: 'Total votings',   value: allVotings.length,   bg: 'bg-slate-100',  tc: 'text-slate-600'    },
            { icon: <CheckCircle2className="w-5 h-5" />, label: 'Resolutions passed', value: passed,            bg: 'bg-emerald-50', tc: 'text-emerald-600' },
            { icon: <XCircle     className="w-5 h-5" />, label: 'Rejected',        value: rejected,            bg: 'bg-red-50',     tc: 'text-red-500'      },
            { icon: <TrendingUp  className="w-5 h-5" />, label: 'Agenda items',    value: agendaItems.length,  bg: 'bg-blue-50',    tc: 'text-blue-600'     },
          ].map((stat, i) => (
            <div key={i} className="bg-white border border-slate-200 rounded-xl p-5 flex items-start gap-4 shadow-sm">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${stat.bg} ${stat.tc}`}>{stat.icon}</div>
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">{stat.label}</p>
                <p className="text-2xl font-extrabold text-slate-900 leading-tight tabular-nums">{stat.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {completed.length === 0 && (
          <div className="text-center py-16 text-slate-400 bg-white border border-slate-200 rounded-2xl">
            ...
            <p className="text-sm mt-1">Statistics will appear after votings are completed</p>
          </div>
        )}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
            {[
              ...
              { label: 'Time',         val: new Date(session.scheduledAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }) },
