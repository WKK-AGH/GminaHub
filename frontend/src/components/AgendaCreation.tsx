...
function Toast({
    ...
    return (
        <div
            className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold ${typ === 'success' ? 'bg-slate-900 text-white' : 'bg-red-600 text-white'}`}
        >
            {typ === 'success' ? (
    ...
    return (
        ...
        >
            ...
            <div className="flex items-start gap-3 px-4 py-4">
                <div className="shrink-0 cursor-grab active:cursor-grabbing text-slate-300 hover:text-slate-500 transition mt-0.5 touch-none">
                ...
                <div className="flex-1 min-w-0">
                    <input
                        type="text"
                        value={punkt.tytul}
                        onChange={(e) => onAktualizuj(punkt.id, { tytul: e.target.value })}
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                        <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full border bg-slate-50 text-slate-600 border-slate-200">
                            <span className={`w-1.5 h-1.5 rounded-full ${cfg.kropkaKlasa}`} />
                            {cfg.etykieta}
                        </span>
                        {punkt.referent && (
                        ...
                        <span className="text-xs text-slate-400 flex items-center gap-1">
                            ...
                            {formatujCzas(punkt.czasMinuty)}
                        </span>
                        {punkt.zalaczniki.length > 0 && (
                            <span className="text-xs text-slate-400 flex items-center gap-1">
                                <Paperclip className="w-3 h-3" />
                                {punkt.zalaczniki.length}
                            </span>
                        )}
                        {punkt.wymagaGlosowania && (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-50 text-[#B91C1C] border border-red-200">
                                Głosowanie
                            </span>
                        )}
                    </div>
            ...
            {rozwiniety && (
                <div className="border-t border-slate-100 px-4 py-4 bg-slate-50/40 space-y-4">
                    <div>
                        ...
                        <textarea
                            value={punkt.opis}
                            onChange={(e) => onAktualizuj(punkt.id, { opis: e.target.value })}
                            placeholder="Opcjonalny opis punktu..."
                            rows={2}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div>
                            ...
                            <select
                                value={punkt.typ}
                                onChange={(e) =>
                                    onAktualizuj(punkt.id, {
                                        typ: e.target.value as TypPunktu,
                                        wymagaGlosowania: e.target.value === 'glosowanie',
                                    })
                                }
                        ...
                        <div className="col-span-2">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    onAktualizuj(punkt.id, { referent: e.target.value })
                                }
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    onAktualizuj(punkt.id, {
                                        czasMinuty: Math.max(1, parseInt(e.target.value) || 1),
                                    })
                                }
                    ...
                    <label className="flex items-center gap-3 cursor-pointer select-none">
                        <div
                            onClick={() =>
                                onAktualizuj(punkt.id, {
                                    wymagaGlosowania: !punkt.wymagaGlosowania,
                                })
                            }
                            className={`w-10 h-6 rounded-full flex items-center px-0.5 transition-colors ${punkt.wymagaGlosowania ? 'bg-[#B91C1C]' : 'bg-slate-200'}`}
                        >
                            <div
                                className={`w-5 h-5 rounded-full bg-white shadow transition-transform ${punkt.wymagaGlosowania ? 'translate-x-4' : 'translate-x-0'}`}
                            />
                        </div>
                    ...
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <button
                                ...
                                className="inline-flex items-center gap-1 text-xs font-bold text-[#B91C1C] hover:text-[#7F1D1D] transition"
                            >
                        ...
                        {punkt.zalaczniki.length === 0 ? (
                            ...
                            >
                                <Paperclip className="w-4 h-4" /> Przeciągnij pliki lub kliknij aby
                                dodać
                            </button>
                        ) : (
                                ...
                                {punkt.zalaczniki.map((z) => (
                                    ...
                                    >
                                        ...
                                        <button
                                            onClick={() =>
                                                onAktualizuj(punkt.id, {
                                                    zalaczniki: punkt.zalaczniki.filter(
                                                        (a) => a.id !== z.id,
                                                    ),
                                                })
                                            }
            ...
            <div className="divide-y divide-slate-50 max-h-80 overflow-y-auto">
                ...
                {punkty.map((p, i) => (
                    <div key={p.id} className="flex items-start gap-3 px-4 py-3">
                        <span className="w-5 h-5 rounded-full bg-slate-100 text-slate-500 text-[10px] font-extrabold flex items-center justify-center shrink-0 mt-0.5">
                            {i + 1}
                        </span>
                        <div className="flex-1 min-w-0">
                            <p className="text-xs font-semibold text-slate-800 truncate">
                                {p.tytul || (
                                    <span className="text-slate-300 italic">Brak tytułu</span>
                                )}
                            </p>
                        ...
                        {p.wymagaGlosowania && (
                            <span className="w-4 h-4 rounded-full bg-[#B91C1C] flex items-center justify-center shrink-0 mt-0.5">
...
export default function AgendaCreation() {
    ...
    const dodajSzablon = (tytul: string) =>
        setPunkty((prev) => [
            ...prev,
            {
                id: uid(),
    ...
    const handleZapisz = async () => {
        ...
        if (puste.length > 0) {
            pokazToast(`${puste.length} punkt(y) ma pusty tytuł`, 'error');
            return;
        }
        setZapisywanie(true);
        try {
            ...
            pokazToast('Błąd zapisu agendy', 'error');
        } finally {
    ...
    const handlePublikuj = async () => {
        const puste = punkty.filter((p) => !p.tytul.trim());
        if (puste.length > 0) {
            pokazToast('Uzupełnij wszystkie tytuły przed publikacją', 'error');
            return;
        }
        if (punkty.length < 2) {
            pokazToast('Agenda musi mieć co najmniej 2 punkty', 'error');
            return;
        }
        setZapisywanie(true);
        try {
            ...
            pokazToast('Błąd publikacji agendy', 'error');
        } finally {
            setZapisywanie(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50">
            {toast && (
                <Toast wiadomosc={toast.msg} typ={toast.typ} onClose={() => setToast(null)} />
            )}
            ...
            <div className="bg-white border-b border-slate-200 px-4 py-7">
                <div className="max-w-6xl mx-auto">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                            <p className="text-xs font-bold text-[#B91C1C] uppercase tracking-widest mb-1.5">
                                Tworzenie agendy
                            </p>
                            ...
                            <div className="flex flex-wrap gap-4 mt-2 text-xs text-slate-500">
                                <span className="flex items-center gap-1.5">
                                    ...
                                    Zaplanuj porządek obrad
                                </span>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() => setPodglad((v) => !v)}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5">
                        {[
                            ...
                            {
                                etykieta: 'Szac. czas',
                                wartosc: formatujCzas(lacznyCzas),
                                cls: 'text-slate-900',
                            },
                        ].map((s) => (
                            <div
                            ...
                            >
                                <p
                                    className={`text-xl font-extrabold tabular-nums leading-none ${s.cls}`}
                                >
                                    {s.wartosc}
                                </p>
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                <div
                    className={`grid gap-6 ${podglad ? 'grid-cols-1 lg:grid-cols-3' : 'grid-cols-1'}`}
                >
                    <div className={podglad ? 'lg:col-span-2' : ''}>
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="font-extrabold text-slate-900 flex items-center gap-2 text-sm">
                                <FileText className="w-4 h-4 text-[#B91C1C]" /> Punkty porządku
                                obrad ({punkty.length})
                            </h2>
                            <div className="flex items-center gap-1.5 text-xs text-slate-400">
                                <GripVertical className="w-3.5 h-3.5" /> Przeciągnij aby zmienić
                                kolejność
                            </div>
                        </div>
                        ...
                        <div className="mt-6 border border-slate-200 rounded-2xl p-4 bg-white">
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">
                                Szybkie dodawanie — typowe punkty
                            </p>
                            <div className="flex flex-wrap gap-2">
                                {SZYBKIE_SZABLONY.map((szablon) => (
                                    <button
                                        key={szablon}
                                        onClick={() => dodajSzablon(szablon)}
                    ...
                    {podglad && (
                        <div className="lg:col-span-1">
                            <PodgladAgendy
                                punkty={punkty}
                                tytulSesji={sessionId ? `Sesja #${sessionId}` : 'Sesja'}
                            />
                        </div>
                    )}
                </div>
