...
export default function Login() {
  ...
  const skad = (location.state as { from?: { pathname: string } })?.from?.pathname ?? '/panel';

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!loginValue.trim() || !haslo.trim()) {
      setBlad('Wypełnij wszystkie pola');
      return;
    }
    setBlad('');
    setLadowanie(true);
    try {
      ...
      navigate(skad === '/login' ? '/panel' : skad, { replace: true });
    } catch (err: unknown) {
      setBlad((err as { message?: string })?.message ?? 'Błąd logowania');
    } finally {
      setLadowanie(false);
    }
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Twój login" value={loginValue}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" placeholder="••••••••" value={haslo}
                  onChange={e => setHaslo(e.target.value)} required autoComplete="current-password"
                  className="w-full pl-9 pr-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
            ...
            <button type="submit" disabled={ladowanie}
              ...
              {ladowanie && <Loader2 className="w-4 h-4 animate-spin" />}
