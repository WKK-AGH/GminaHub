...
export default function SessionCreation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !date || !time) { setError('Proszę wypełnić wszystkie wymagane pola'); return; }
    setError(null);
    setLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-7">
        <div className="max-w-2xl mx-auto">
          <p className="text-xs font-semibold text-[#B91C1C] uppercase tracking-widest mb-1">Nowa sesja</p>
      ...
      <div className="max-w-2xl mx-auto px-4 py-7">
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded p-6 space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1.5">Tytuł sesji *</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1.5">Godzina *</label>
          ...
          <div className="flex items-start gap-3 bg-slate-50 border border-slate-200 rounded p-4 text-sm text-slate-600">
            ...
            <div>
              <p className="font-semibold mb-1 text-slate-700">Co dzieje się po utworzeniu?</p>
          ...
          <ol className="space-y-2 text-sm text-slate-500">
            {[
              ...
              <li key={i} className="flex items-start gap-2.5">
                <span className="w-5 h-5 rounded-full bg-[#B91C1C] text-white font-bold text-xs flex items-center justify-center flex-shrink-0 mt-0.5">{i + 1}</span>
                {step}
              </li>
            ))}
          </ol>
