...
export const refresh = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (isNaN(numericUserId)) {
            res.status(401).json({
                success: false,
                message: 'Nieprawidłowy format identyfikatora w tokenie',
            });
            return;
        }
...
export const register = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (!role) {
            res.status(400).json({
                success: false,
                message: `Rola '${roleName}' nie istnieje w systemie`,
            });
            return;
        }
        ...
        logger.info(
            `ADMIN pomyślnie zarejestrował nowego użytkownika: ${newUser.email} (Rola: ${newUser.role.name})`,
        );
