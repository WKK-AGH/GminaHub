...
export const initWebsockets = (io: Server) => {
    ...
    io.on('connection', (socket: AuthenticatedSocket) => {
        ...
        socket.on('join_session', async ({ sessionId }) => {
            ...
            console.log(
                `Użytkownik ${userId} dołączył do pokoju sesji: session_${numericSessionId}`,
            );
        ...
        socket.on('start_voting', async ({ votingId, sessionId }) => {
            if (socket.user?.role !== 'CHAIRPERSON' && socket.user?.role !== 'ADMIN') {
        ...
        socket.on('cast_vote', async ({ votingId, choice, sessionId }) => {
            ...
            try {
                ...
                io.to(`session_${numericSessionId}`).emit('vote_updated', {
                    votingId: numericVotingId,
                    results,
                });
            } catch (err) {
